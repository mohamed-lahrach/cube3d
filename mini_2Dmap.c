...
void draw_rays(t_game *game, float factor)
{
    for (int i = 0; i < NUM_RAYS; i++)
    {
        draw_line(
            game->img_data, 
            factor * game->player.x, 
            factor * game->player.y, 
            factor * game->rays[i].wall_hit_x, 
            factor * game->rays[i].wall_hit_y, 
            RED_COLOR, 
            game->size_line, 
            game->bpp, 
            0.3
...
void render_minimap(t_game *game)
{
    ...
    for (int i = 0; i < MAP_HEIGHT; i++)
    {
        for (int j = 0; j < MAP_WIDTH; j++)
        {
            ...
            draw_square(game->img_data, minimap_x, minimap_y, minimap_tile_size, color, game->size_line, game->bpp);
        }
    }
    ...
    int player_minimap_radius = (int)(game->player.radius * MINIMAP_SCALE_FACTOR);

    draw_circle(game->img_data, player_minimap_x, player_minimap_y, player_minimap_radius, RED_COLOR, game->size_line, game->bpp);
    ...
    for (int i = 0; i < NUM_RAYS; i++)
    {
        ...
        if (1)
        {
            ...
            int ray_end_x = (int)(game->rays[i].wall_hit_x * MINIMAP_SCALE_FACTOR);
            int ray_end_y = (int)(game->rays[i].wall_hit_y * MINIMAP_SCALE_FACTOR);

            // Draw the ray as a line in the mini-map
            draw_line(game->img_data, ray_start_x, ray_start_y, ray_end_x, ray_end_y, RED_COLOR, game->size_line, game->bpp, 0.3);
        }
    }
}